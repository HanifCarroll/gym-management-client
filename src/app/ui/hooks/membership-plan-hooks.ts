...
const membershipPlanService = new MembershipPlanServiceImpl(membershipPlanRepository);
...
export const useCreateMembershipPlan = () => {
  ...
  return useMutation<MembershipPlan, Error, CreateMembershipPlanData>({
    mutationFn: (newPlan) => membershipPlanService.createMembershipPlan(newPlan),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: [ 'membershipPlans' ] });
      showSnackbar('Membership plan created successfully!', 'success');
    },
    onError: (error) => {
      showSnackbar('Failed to create membership plan. Please try again.', 'error');
...
export const useUpdateMembershipPlan = () => {
  ...
  return useMutation<MembershipPlan, Error, { id: string; plan: UpdateMembershipPlanData }>({
    mutationFn: ({ id, plan }) => membershipPlanService.updateMembershipPlan(id, plan),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: [ 'membershipPlans' ] });
      showSnackbar('Membership plan updated successfully!', 'success');
    },
    onError: (error) => {
      showSnackbar('Failed to update membership plan. Please try again.', 'error');
...
export const useDeleteMembershipPlan = () => {
  ...
  return useMutation<void, Error, string>({
    ...
    onError: (error) => {
      showSnackbar('Failed to delete membership plan. Please try again.', 'error');
